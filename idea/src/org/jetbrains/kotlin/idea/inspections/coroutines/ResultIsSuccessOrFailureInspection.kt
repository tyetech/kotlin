...
class ResultIsSuccessOrFailureInspection : AbstractKotlinInspection() {
    ...
    private fun MemberScope.hasCorrespondingNonCatchingFunction(
        ...
        return nonCatchingFunctions.any { nonCatchingFun ->
            nonCatchingFun.valueParameters.size == valueParameters.size && nonCatchingFun.returnType == returnType
        }
    }
    ...
    private fun FunctionDescriptor.hasCorrespondingNonCatchingFunction(returnType: KotlinType, nameWithoutCatching: String): Boolean? {
        ...
        if (scope.hasCorrespondingNonCatchingFunction(nameWithoutCatching, valueParameters, returnTypeArgument)) {
            return true
        }
        val extensionReceiver = this.extensionReceiverParameter
        if (extensionReceiver != null) {
            val extensionClassDescriptor = extensionReceiver.type.constructor.declarationDescriptor as? ClassDescriptor
            if (extensionClassDescriptor != null) {
                val extensionClassScope = extensionClassDescriptor.unsubstitutedMemberScope
                if (extensionClassScope.hasCorrespondingNonCatchingFunction(nameWithoutCatching, valueParameters, returnTypeArgument)) {
                    return true
                }
            }
        }
    ...
    private fun analyzeFunction(function: KtFunction, toReport: PsiElement, holder: ProblemsHolder) {
        ...
        if (function is KtFunctionLiteral || returnTypeText == null) {
            ...
            if (text != null && ALLOWED_NAMES.none { it in text } && SHORT_NAME !in text) return
        }
        ...
        if (name != null && name.endsWith(CATCHING)) {
            ...
            val fixes = mutableListOf<LocalQuickFix>(AddGetOrThrowFix(withBody = function.hasBody()))
            if (name != null) {
                fixes += RenameToCatchingFix(name + CATCHING)
            }
            holder.registerProblem(
                toReport,
